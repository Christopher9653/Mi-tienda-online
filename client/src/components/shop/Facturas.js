...
import { Container, Row, Col, Card, Table, Alert, Spinner, Button, Modal, Badge } from 'react-bootstrap';
...
function Facturas() {
  ...
  const cargarFacturas = async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:3001/api/facturas/usuario/${decoded.id}`, {
  ...
  const verDetalleFactura = async (factura) => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`http://localhost:3001/api/facturas/detalle/${factura.factura_id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const descargarFacturaPDF = async (factura) => {
    try {
      ...
      const contenidoHTML = `
        <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 800px;">
          <div style="text-align: center; border-bottom: 2px solid #007bff; padding-bottom: 20px; margin-bottom: 30px;">
            ...
            <p style="color: #666; margin: 5px 0;">Sistema de Facturación</p>
          </div>
          
          <div style="display: flex; justify-content: space-between; margin-bottom: 30px;">
            <div>
              <h3 style="color: #333; margin-bottom: 10px;">FACTURA</h3>
              <p><strong>Número:</strong> ${facturaCompleta.numero_factura || `#${facturaCompleta.factura_id}`}</p>
          ...
          <div style="margin-top: 40px; text-align: center; color: #666; font-size: 12px;">
            ...
            <p>Mi Tienda Online - Sistema de Facturación</p>
          </div>
        </div>
      `;
      ...
      // Descargar PDF
      pdf.save(`factura-${facturaCompleta.numero_factura || facturaCompleta.factura_id}.pdf`);
  ...
  if (loading) {
    return (
      <Container className="mt-5 text-center">
        ...
        <p className="mt-3 text-muted">Cargando tus facturas...</p>
      </Container>
    );
  }
    ...
    <Container className="mt-4">
        ...
        <Col>
          <div className="d-flex justify-content-between align-items-center">
            <div>
              ...
              <p className="text-muted mb-0">Gestiona y descarga tus facturas de compra</p>
            </div>
        ...
        <Row className="mb-4">
          <Col md={4}>
            <Card className="border-0 shadow-sm h-100" style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
              <Card.Body className="text-center text-white">
                ...
                <Card.Title className="h4 mb-2">Total Gastado</Card.Title>
                <Card.Text className="h3 mb-0">
                  ${Number(facturas.reduce((total, factura) => total + Number(factura.total || 0), 0)).toFixed(2)}
                </Card.Text>
              </Card.Body>
            </Card>
          </Col>
          <Col md={4}>
            <Card className="border-0 shadow-sm h-100" style={{ background: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' }}>
              <Card.Body className="text-center text-white">
                <i className="bi bi-calendar-check h1 mb-3"></i>
                <Card.Title className="h4 mb-2">Última Compra</Card.Title>
                <Card.Text className="h5 mb-0">
                  {facturas.length > 0 ? new Date(facturas[0].fecha).toLocaleDateString() : 'N/A'}
                </Card.Text>
              </Card.Body>
            </Card>
          </Col>
          <Col md={4}>
            <Card className="border-0 shadow-sm h-100" style={{ background: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' }}>
              <Card.Body className="text-center text-white">
                <i className="bi bi-clock-history h1 mb-3"></i>
                <Card.Title className="h4 mb-2">Promedio por Factura</Card.Title>
                <Card.Text className="h5 mb-0">
                  ${facturas.length > 0 ? (Number(facturas.reduce((total, factura) => total + Number(factura.total || 0), 0)) / facturas.length).toFixed(2) : '0.00'}
                </Card.Text>
              </Card.Body>
            </Card>
          </Col>
        ...
        <Card.Body className="p-0">
          {facturas.length > 0 ? (
              ...
              <Table className="mb-0">
                ...
                <tbody>
                  {facturas.map((factura, index) => (
                    <tr key={factura.factura_id} className={index % 2 === 0 ? 'bg-white' : 'bg-light'}>
                      <td className="py-3 px-4">
                        <div className="d-flex align-items-center">
                          ...
                          <div>
                            <strong className="text-dark">{factura.numero_factura || `#${factura.factura_id}`}</strong>
                          </div>
                        </div>
                      </td>
                      <td className="py-3">
            ...
            <div className="text-center py-5">
              ...
              <h4 className="text-muted mb-3">No tienes facturas aún</h4>
              <p className="text-muted mb-4">Cuando realices tu primera compra, aquí aparecerán tus facturas</p>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" centered>
        <Modal.Header closeButton className="bg-primary text-white">
          <Modal.Title>
            <i className="bi bi-receipt me-2"></i>
            Detalle de Factura {selectedFactura?.numero_factura || `#${selectedFactura?.factura_id}`}
          </Modal.Title>
        </Modal.Header>
              ...
              <Row className="mb-4">
                <Col md={6}>
                  <div className="bg-light p-3 rounded">
                    ...
                    <p className="mb-1"><strong>Número:</strong> {selectedFactura.numero_factura || `#${selectedFactura.factura_id}`}</p>
